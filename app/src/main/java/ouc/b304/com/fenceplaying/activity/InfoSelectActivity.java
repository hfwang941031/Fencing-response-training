package ouc.b304.com.fenceplaying.activity;

import android.app.Activity;
import android.content.Context;
import android.content.Intent;
import android.os.Bundle;
import android.support.annotation.Nullable;
import android.view.View;
import android.widget.ImageView;
import android.widget.RelativeLayout;
import android.widget.TextView;

import butterknife.BindView;
import butterknife.ButterKnife;
import butterknife.OnClick;
import ouc.b304.com.fenceplaying.R;

/**
 * @author 王海峰 on 2019/1/11 09:33
 */
public class InfoSelectActivity extends Activity {
    @BindView(R.id.img_icon_responsetrain)
    ImageView imgIconResponsetrain;
    @BindView(R.id.tv_playerinfo)
    TextView tvPlayerinfo;
    @BindView(R.id.rl_playerinfo)
    RelativeLayout rlPlayerinfo;
    @BindView(R.id.img_icon_accuracytrain)
    ImageView imgIconAccuracytrain;
    @BindView(R.id.tv_coachinfo)
    TextView tvCoachinfo;
    @BindView(R.id.rl_coachinfo)
    RelativeLayout rlCoachinfo;

    private Context context;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_infoselect);
        ButterKnife.bind(this);
        context = this.getApplicationContext();
    }

    @Override
    protected void onStart() {
        super.onStart();
    }

    @Override
    protected void onRestart() {
        super.onRestart();
    }

    @Override
    protected void onResume() {
        super.onResume();
    }

    @Override
    protected void onPause() {
        super.onPause();
    }

    @Override
    protected void onStop() {
        super.onStop();
    }

    @Override
    protected void onDestroy() {
        super.onDestroy();
    }

    @OnClick({R.id.rl_playerinfo, R.id.rl_coachinfo})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.rl_playerinfo:
                startActivity(new Intent(context, PlayerShowActivity.class));

                break;
            case R.id.rl_coachinfo:
                startActivity(new Intent(context, CoachShowActivity.class));

                break;
        }
    }
}
